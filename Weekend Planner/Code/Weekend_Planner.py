...
def _candidate_richness(c: dict, weekend_isos: frozenset = frozenset()) -> float:
    ...
    primary Notion `Date` lands inside the target weekend gets a dominating
    bonus, so when two cross-source rows for the same event collide we keep
    the one actually dated to the weekend rather than the one with the longest
    blurb. (The scrapers keep each event's URL aligned to its date upstream,
    so we no longer need to parse the date out of the URL here.)
    """
    score = 0.0
    ...
    if d[:10] in weekend_isos:
        score += 100.0   # in-weekend primary date wins the tiebreak
